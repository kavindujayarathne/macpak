...
# 1 = prefer user Trash (default), 0 = permanent remove
USE_TRASH=${USE_TRASH:-1}

# Brewfile update (yes=1, no=0)
AUTO_BREWFILE=${AUTO_BREWFILE:-1}
...
# 24h
INDEX_TTL_SECS=${INDEX_TTL_SECS:-86400}

# Use cached index even when a query is supplied (1=yes, 0=no)
...
Subcommands:
  search [query]                  Search available Homebrew formulae and casks; Enter to install
  list [query]                    List installed Homebrew formulae and casks; Enter to uninstall
